...
float4 main(VSOutput vout) : SV_TARGET
{
    ...
    // Base + 1
    samplingTexcoords.texcoord0.xy = float2(1.0f / texSize.x, 0.0f);

    // Base + 3
    samplingTexcoords.texcoord1.xy = float2(3.0f / texSize.x, 0.0f);

    // Base + 5
    samplingTexcoords.texcoord2.xy = float2(5.0f / texSize.x, 0.0f);

    // Base + 7
    samplingTexcoords.texcoord3.xy = float2(7.0f / texSize.x, 0.0f);

    // Base + 9
    samplingTexcoords.texcoord4.xy = float2(9.0f / texSize.x, 0.0f);

    // Base + 11
    samplingTexcoords.texcoord5.xy = float2(11.0f / texSize.x, 0.0f);

    // Base + 13
    samplingTexcoords.texcoord6.xy = float2(13.0f / texSize.x, 0.0f);

    // Base + 15
    samplingTexcoords.texcoord7.xy = float2(15.0f / texSize.x, 0.0f);
